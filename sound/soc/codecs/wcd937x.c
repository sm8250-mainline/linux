...
static struct regmap_irq_chip wcd937x_regmap_irq_chip = {
	...
	.use_ack = 1,
	.clear_ack = 1,
	// .config_base = WCD937X_DIGITAL_INTR_LEVEL_0,
	.runtime_pm = true,
...
static void wcd937x_io_init(struct regmap *regmap)
{
	...
	regmap_update_bits(regmap, WCD937X_LDORXTX_CONFIG, 0x10, 0x00);
	regmap_update_bits(regmap, WCD937X_BIAS_VBG_FINE_ADJ, 0xf0, 0x80);
	regmap_update_bits(regmap, WCD937X_ANA_BIAS, 0x80, 0x80);
	regmap_update_bits(regmap, WCD937X_ANA_BIAS, 0x40, 0x40);
	usleep_range(10000, 10010);

	regmap_update_bits(regmap, WCD937X_ANA_BIAS, 0x40, 0x00);
...
static int wcd937x_rx_clk_enable(struct snd_soc_component *component)
{
	...
	snd_soc_component_update_bits(component, WCD937X_DIGITAL_CDC_DIG_CLK_CTL, 0x08, 0x08);
	snd_soc_component_update_bits(component, WCD937X_DIGITAL_CDC_ANA_CLK_CTL, 0x01, 0x01);
	snd_soc_component_update_bits(component, WCD937X_ANA_RX_SUPPLIES, 0x01, 0x01);
	snd_soc_component_update_bits(component, WCD937X_DIGITAL_CDC_RX0_CTL, 0x40, 0x00);
	snd_soc_component_update_bits(component, WCD937X_DIGITAL_CDC_RX1_CTL, 0x40, 0x00);
	snd_soc_component_update_bits(component, WCD937X_DIGITAL_CDC_RX2_CTL, 0x40, 0x00);
	snd_soc_component_update_bits(component, WCD937X_DIGITAL_CDC_ANA_CLK_CTL, 0x02, 0x02);
...
static int wcd937x_rx_clk_disable(struct snd_soc_component *component)
{
	...
	snd_soc_component_update_bits(component, WCD937X_ANA_RX_SUPPLIES, 0x01, 0x00);
	snd_soc_component_update_bits(component, WCD937X_DIGITAL_CDC_ANA_CLK_CTL, 0x02, 0x00);
	snd_soc_component_update_bits(component, WCD937X_DIGITAL_CDC_ANA_CLK_CTL, 0x01, 0x00);
				       ...
				       int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		snd_soc_component_update_bits(component, WCD937X_FLYBACK_EN, 0x04, 0x00);
				   ...
				   int req_volt, int micb_num)
{
	...
	if (micb_val == WCD937X_MICB_ENABLE)
		snd_soc_component_update_bits(component, micb_reg, WCD937X_ANA_MICB_EN, 0x80);

	snd_soc_component_update_bits(component, micb_reg, WCD937X_ANA_MICB_VOUT, req_vout_ctl);

	if (micb_val == WCD937X_MICB_ENABLE) {
		snd_soc_component_update_bits(component, micb_reg, WCD937X_ANA_MICB_EN, 0x40);
		/* 2ms delay after enabling micbias, as per HW requirement */
			       ...
			       struct wcd_mbhc_config *mbhc_cfg)
{
	if (mbhc_cfg->enable_usbc_analog)
		if (!(snd_soc_component_read(component, WCD937X_ANA_MBHC_MECH) & BIT(5)))
			return true;
						 ...
						 int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		break;
	case SND_SOC_DAPM_POST_PMU:
		/* 1 ms delay as per HW requirement */
		usleep_range(1000, 1100);
		break;
	case SND_SOC_DAPM_POST_PMD:
...
static int wcd937x_soc_codec_probe(struct snd_soc_component *component)
{
	...
	wcd937x->variant = (snd_soc_component_read(component, WCD937X_DIGITAL_EFUSE_REG_0) & 0x1E) >> 1;
...
static int wcd937x_suspend(struct device *dev)
{
	...
	if (test_bit(ALLOW_BUCK_DISABLE, &wcd937x->status_mask)) {
		ret = regulator_disable(wcd937x->buck_supply);
		if (ret) {
			dev_err(dev, "VDD_BUCK is not disabled\n");
			return ret;
		}
...
static int wcd937x_populate_dt_data(struct wcd937x_priv *wcd937x, struct device *dev)
{
	struct wcd_mbhc_config *cfg = &wcd937x->mbhc_cfg;
	int ret;
	...
	// wcd937x->us_euro_gpio = devm_gpiod_get_optional(dev, "us-euro",
	// 					GPIOD_OUT_LOW);
...
static int wcd937x_probe(struct platform_device *pdev)
{
	...
	wcd937x = kzalloc(sizeof(*wcd937x), GFP_KERNEL);
	if (!wcd937x)
		return -ENOMEM;
	...
	ret = wcd937x_populate_dt_data(wcd937x, dev);
	if (ret) {
		dev_err(dev, "Failed to obtain platform data\n");
		return ret;
	}
